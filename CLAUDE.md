...
- `scripts/nlcs2csv.py` — the converter (only real code so far). Python, needs pyproj.
- `resources/` — converted map-ready output, one folder per source file. **Must remain
  byte-identically reproducible** from the converter (see below).
...
for f in enexis_voorbeeld_3092025_1554 xml_revisie xml_revisie_2; do
  .venv/bin/python scripts/nlcs2csv.py data/$f.xml resources/$f --per-category
done
